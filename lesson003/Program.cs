...
int[] MadeArray(int number)
{
    ...
    int[] array = new int[GetLengthNumber(number)];

    for (int i=GetLengthNumber(number)-1;i>=0;i--)//0
    {
    array[i]=number%10;//4
    number/=10;//0
    }
...
// }
PrintArray(MadeArray(N));
Console.WriteLine();
PrintArray(MadeArray2(N));
Console.WriteLine(GetLengthNumber(N));
   int[] result = MadeArray(N);
    int[] result2 = MadeArray2(N);
    //bool result3 = Enumerable.SequenceEqual(result, result2);
    bool result3 = MadeArray(N).SequenceEqual(MadeArray2(N));
    Console.WriteLine(result3);

if (GetLengthNumber(N)==5 & result3)
//{
